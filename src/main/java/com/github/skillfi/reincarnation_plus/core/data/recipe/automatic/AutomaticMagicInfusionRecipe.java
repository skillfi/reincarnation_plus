package com.github.skillfi.reincarnation_plus.core.data.recipe.automatic;
...
public class AutomaticMagicInfusionRecipe extends AutomaticMagicInfuserRecipe implements Comparable<AutomaticMagicInfusionRecipe> {
    private static final Logger log = LogManager.getLogger(AutomaticMagicInfusionRecipe.class);
    public static final ResourceLocation EMPTY = new ResourceLocation("minecraft:air");
    public static final ResourceLocation MAGICULES = new ResourceLocation(ReiMod.MODID, "magicules");
    ...
    public static final ResourceLocation EXP = new ResourceLocation(ReiMod.MODID, "experience");
    @Getter
    ...
    public boolean matches(AutomaticMagiculaInfuserBlockEntity pContainer, Level level) {
    ...
    public void infuse(AutomaticMagiculaInfuserBlockEntity pContainer, ResourceLocation type, float amount) {
    ...
    public FinishedRecipe finishRecipe() {
        return new FinishedRecipe() {
            public void serializeRecipeData(JsonObject root) {
                if (!AutomaticMagicInfusionRecipe.this.getMagiculesId().equals(AutomaticMagicInfusionRecipe.EMPTY)) {
                    JsonObject input = new JsonObject();
                    input.addProperty("id", AutomaticMagicInfusionRecipe.this.magiculesId.toString());
                    input.addProperty("amount", AutomaticMagicInfusionRecipe.this.magicules);
                    root.add("magicules", input);
                }

                if (!AutomaticMagicInfusionRecipe.this.input.isEmpty()) {
                    root.add("input", AutomaticMagicInfusionRecipe.this.input.toJson());
                }
                root.addProperty("cookingTime", AutomaticMagicInfusionRecipe.this.cookingTime);
                root.add("result", (JsonElement)ItemStack.CODEC.encodeStart(JsonOps.INSTANCE, AutomaticMagicInfusionRecipe.this.output).result().orElseThrow(() -> new IllegalArgumentException("Could not serialize ItemStack: " + AutomaticMagicInfusionRecipe.this.output)));
            }

            public ResourceLocation getId() {
                return AutomaticMagicInfusionRecipe.this.id;
            }

            public RecipeSerializer<?> getType() {
                return AutomaticMagicInfusionRecipe.this.getSerializer();
            }
    ...
    public RecipeSerializer<?> getSerializer() {
        return ReiRecipeTypes.Serializer.AUTOMATIC_MAGIC_INFUSION.get();
    }

    public RecipeType<?> getType() {
        return ReiRecipeTypes.AUTOMATIC_MAGIC_INFUSION.get();
    }

    public int compareTo(@NotNull AutomaticMagicInfusionRecipe o) {
    ...
    public boolean equals(Object o) {
        if (this == o) {
            ...
            AutomaticMagicInfusionRecipe recipe = (AutomaticMagicInfusionRecipe)o;
    ...
    public AutomaticMagicInfusionRecipe(ResourceLocation id, ResourceLocation magiculesId, float magicules, Ingredient ingredient, int cookingTime, ItemStack output) {
        this.id = id;
    ...
    public static class Serializer implements RecipeSerializer<AutomaticMagicInfusionRecipe> {
        public Serializer() {
        }

        public AutomaticMagicInfusionRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
            ResourceLocation inputLeft = AutomaticMagicInfusionRecipe.EMPTY;
            ...
            return new AutomaticMagicInfusionRecipe(pRecipeId, inputLeft, inputLeftAmount, input, infusionTime, output);
        }

        public @Nullable AutomaticMagicInfusionRecipe fromNetwork(ResourceLocation pRecipeId, FriendlyByteBuf pBuffer) {
            return new AutomaticMagicInfusionRecipe(pRecipeId, pBuffer.readResourceLocation(), pBuffer.readInt(), Ingredient.fromNetwork(pBuffer), pBuffer.readInt(), pBuffer.readItem());
        }

        public void toNetwork(FriendlyByteBuf pBuffer, AutomaticMagicInfusionRecipe pRecipe) {
    ...
    public static class Type implements RecipeType<AutomaticMagicInfusionRecipe> {
        public Type() {
        }
    }
    ...
    public static class Builder {
        ...
        public void build(Consumer<FinishedRecipe> consumer, ResourceLocation id) {
            consumer.accept((new AutomaticMagicInfusionRecipe(new ResourceLocation(id.getNamespace(), "automatic_magic_infusion/" + id.getPath()), this.magiculesId, this.magicules, this.input == null ? Ingredient.EMPTY : this.input, this.infusionTime, this.output)).finishRecipe());
        }
        ...
        private Builder(ItemStack output) {
            this.magiculesId = AutomaticMagicInfusionRecipe.EMPTY;
            this.magicules = 0;
