...
public class AutomaticMagicInfuserScreen extends AbstractContainerScreen<AutomaticMagicInfuserMenu> {
...
//    private final ImagePredicateButton infusion;
     ...
     */
    public AutomaticMagicInfuserScreen(AutomaticMagicInfuserMenu menu, Inventory inventory, Component component) {
        super(menu, inventory, component);

        // Ініціалізація матеріалів
        this.leftBarMaterial = resolveBarMaterial(menu.blockEntity,  true);
        this.rightBarMaterial = resolveBarMaterial(menu.blockEntity,  false);
     ...
     */
    private Optional<MagicInfuserMoltenMaterial> resolveBarMaterial(BlockEntity blockEntity, boolean isLeft) {
    ...
    private Optional<MagicInfuserMoltenMaterial> getBarMaterial(BlockEntity entity, boolean isLeft) {
        return isLeft
                ? ((AutomaticMagiculaInfuserBlockEntity) entity).getLeftBarId().flatMap(location -> filterMoltenMaterial(location))
                : ((AutomaticMagiculaInfuserBlockEntity) entity).getRightBarId().flatMap(location -> filterMoltenMaterial(location));
...
//        this.addRenderableWidget(this.infusion);
    ...
    private void renderFire(PoseStack pPoseStack, int x, int y) {
        if (((AutomaticMagicInfuserMenu)this.menu).hasFuel()) {
            this.blit(pPoseStack, x + 37, y + 89 - ((AutomaticMagicInfuserMenu)this.menu).getScaledFuelProgress(), 78, 181 - ((AutomaticMagicInfuserMenu)this.menu).getScaledFuelProgress(), 13, ((AutomaticMagicInfuserMenu)this.menu).getScaledFuelProgress());
        }
    }
    ...
    protected void renderTooltip(PoseStack pPoseStack, int pMouseX, int pMouseY) {
        ...
        if (this.isHovering(220, 5, 15, 76, (double)pMouseX, (double)pMouseY)) {
            ...
            if (!((AutomaticMagicInfuserMenu)this.menu).blockEntity.getRightBarId().equals(Optional.of(MagicInfusionRecipe.EMPTY))
                    && ((AutomaticMagicInfuserMenu)this.menu).blockEntity.getInfusionTime() > 0) {
                // Формує текстове значення для відображення рівня магічного матеріалу.
                float infusionTime = (float)((AutomaticMagicInfuserMenu)this.menu).blockEntity.getInfusionTime();
