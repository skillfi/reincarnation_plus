...
public class OniEntity extends KijinEntity implements IAnimatable {
    ...
    public static AttributeSupplier setAttributes() {
        return Mob.createMobAttributes().
                ...
                add(Attributes.ATTACK_SPEED, ONI.getRace().getBaseAttackSpeed()).
                add(Attributes.MOVEMENT_SPEED, ONI.getRace().getMovementSpeed()).
                add(TensuraAttributeRegistry.MAX_MAGICULE.get(), ONI.getRace().getMaxBaseMagicule()).
    ...
    public @NotNull AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        OniEntity baby = (OniEntity) ((EntityType<?>) ReiEntities.ONI.get()).create(pLevel);
        if (baby == null) {
            return null;
        } else {
