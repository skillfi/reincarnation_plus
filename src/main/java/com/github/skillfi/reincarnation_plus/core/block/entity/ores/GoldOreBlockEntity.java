...
public class GoldOreBlockEntity extends BlockEntity implements IAnimatable {
    ...
    public GoldOreBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ReiBlockEntities.GOLD_MAGIC_ORE.get(), pPos, pBlockState);
        this.tickCounter = 0;
        this.magicules = 0.0;
        this.id = 1;
    ...
    public MagicOreVariant.OreAge getAge(){
        return MagicOreVariant.OreAge.byId(getBlockState().getValue(GoldMagicOreBlock.AGE));
    }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, GoldOreBlockEntity pEntity) {
            ...
            if (pEntity.tickCounter >= TICKS_PER_ATTEMPT) {
                ...
                if (AuraAPI.consumeAura(level, pos, 500.0)){
                    ...
                    if (level.random.nextDouble() < GROWTH_CHANCE && pEntity.getMagicules() > 10000) {
                        pEntity.getBlockState().setValue(GoldMagicOreBlock.AGE, 1);
                        pEntity.needUpdate = true;
                    }
                    if (level.random.nextDouble() < GROWTH_CHANCE && pEntity.getMagicules() > 20000) {
                        pEntity.getBlockState().setValue(GoldMagicOreBlock.AGE, 2);
                        pEntity.needUpdate = true;
                    }
                    if (level.random.nextDouble() < GROWTH_CHANCE && pEntity.getMagicules() > 30000) {
                        pEntity.getBlockState().setValue(GoldMagicOreBlock.AGE, 3);
                        pEntity.needUpdate = true;
                    }
                }
            }
