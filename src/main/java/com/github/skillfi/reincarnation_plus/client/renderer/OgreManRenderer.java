...
public class OgreManRenderer extends MobRenderer<OgreManEntity, ModelOgre<OgreManEntity>> {
	public OgreManRenderer(EntityRendererProvider.Context context) {
		...
		this.addLayer(new OgreArmorLayer(this, new ModelOgre(context.bakeLayer(ModelLayers.PLAYER_INNER_ARMOR)), new ModelOgre(context.bakeLayer(ModelLayers.PLAYER_OUTER_ARMOR))));
	}
